...
当发送的数据长度足够大时，可能无法一次将所有数据发送完毕。此时 `recv` 与 `send` 都可能只收发了部分数据，请处理这种情况，使得长度较大的数据能被完整地发送到客户端。
...
`io_uring` 是最近才被引入内核的高效异步实现。和上面的几种方法相比使用 `io_uring` 能够带来显著的性能提升。不过由于本身是较新的概念，助教也没有完成示例代码，不过你可以参考这里的实现：https://juejin.cn/post/7074212680071905311
...
本次实验满分为 14 分，可获得的分数为 18 分，每个实现的分数都已经标在了标题后。完成一部分进阶实现可以一并获得对应的普通实现的分数。你只需要从以上项目中任选几项完成，超过 14 分的部分可以填补本实验其他项目的扣分，你可以多拿一些分数来让结果更稳一点，但总分不会超过 14。
