...
在使用 shell（这里特指 bash）的时候输入 `history n` 可以显示最近执行的 n 条指令（对于 zsh 用户，`history` 命令会被自动替换为内建的 `fc -l`，显示从 n 条命令开始的记录）。同时可以使用 `!n` 来执行最近的第 n 条指令，同时该命令不会被记入 history。在 shell 中我们还可以通过上下方向键来切换到不同的历史命令。
...
- shell 部分：必做项目共 6 分。对于额外的选做项目，shell 部分总分不超过 8 分：
  ...
  - 你的 shell 支持 `history n` 命令（1 分）
    - 你的 shell 支持通过 `!` 号调用历史命令（0.5 分）
    - 你的 shell 支持通过上下键切换历史命令（0.5 分）
